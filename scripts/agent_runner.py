...
## Proof-of-Work (Save files locally)
Save evidence of your work as files in the project:
- **Screenshots**: Save UI states to .png files
- **Logs**: Save command output to .log files
- **Reports**: Save findings to .md or .json files

Place proof files in a `_proofs/` directory within the project.
Do NOT call any external APIs to upload proofs.
...
class GooseProvider(AgentProvider):
    ...
    def run_agent(self, role: str, run_id: int, project_path: str, prompt: str, task_id: int = None, retry_count: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            # Build environment with Goose context settings
            goose_env = os.environ.copy()
            # Ensure Goose context limits are set from our .env configuration
            goose_context_size = os.getenv("LLM_CONTEXT_SIZE", "250000")
            goose_env.setdefault("GOOSE_CONTEXT_LIMIT", goose_context_size)
            goose_env.setdefault("GOOSE_PLANNER_CONTEXT_LIMIT", goose_context_size)
            goose_env.setdefault("GOOSE_WORKER_CONTEXT_LIMIT", goose_context_size)
            goose_env.setdefault("GOOSE_AUTO_COMPACT_THRESHOLD", os.getenv("GOOSE_AUTO_COMPACT_THRESHOLD", "0.8"))
            goose_env.setdefault("GOOSE_MAX_TURNS", os.getenv("GOOSE_MAX_TURNS", "500"))
            ...
            if self._is_context_limit_error(output):
                if retry_count < MAX_CONTEXT_RETRIES:
                    print(f"  Context limit hit! Attempting to fix and retry ({retry_count + 1}/{MAX_CONTEXT_RETRIES})...")

                    # Try to fix context size configuration
                    try:
                        from app.services.llm_service import fix_context_size_after_error
                        fix_result = fix_context_size_after_error()
                        if fix_result.get("success"):
                            print(f"  Context fix applied: {fix_result.get('message')}")
                        else:
                            print(f"  Context fix failed: {fix_result.get('message')}")
                    except Exception as e:
                        print(f"  Could not apply context fix: {e}")
                    ...
                    # Max retries exceeded - return failure with diagnostic info
                    return {
                        "status": "fail",
                        "summary": f"Context limit exceeded after {retry_count} retry(s). Check LLM_CONTEXT_SIZE in .env",
                        "details": {
                            "error": "context_limit_exceeded",
                            "retries": retry_count,
                            "session_cleared": True,
                            "hint": "Run: docker model configure --context-size=250000 ai/qwen3-coder"
                        },
                        "raw_output": output
                    }
...
# =============================================================================
# Open Interpreter Provider Implementation
# =============================================================================

class OpenInterpreterProvider(AgentProvider):
    """Provider implementation using Open Interpreter.

    Much simpler than Goose - direct LLM calls with code execution.
    No bloated system prompts, no extension overhead.
    """
    ...
    def _configure(self):
        """Configure Open Interpreter for local LLM via Docker Model Runner."""
        # Docker Model Runner exposes OpenAI-compatible API
        llm_base = os.getenv("LLM_BASE_URL", "http://localhost:12434/engines/llama.cpp/v1")
        model_name = os.getenv("GOOSE_MODEL", "ai/qwen3-coder:latest")

        # Configure for OpenAI-compatible local model
        # Use openai/ prefix with custom api_base for local endpoints
        self.interpreter.llm.model = f"openai/{model_name}"
        self.interpreter.llm.api_base = llm_base
        self.interpreter.llm.api_key = "not-needed"  # Local model doesn't need key

        # Performance settings
        self.interpreter.auto_run = True  # Don't ask for confirmation
        self.interpreter.offline = True   # No telemetry
        self.interpreter.llm.supports_functions = False  # Use code blocks instead
        self.interpreter.llm.supports_vision = False
        self.interpreter.llm.context_window = int(os.getenv("LLM_CONTEXT_SIZE", "250000"))
        self.interpreter.llm.max_tokens = 4096

        # Minimal system message - no bloat
        self.interpreter.system_message = ""  # We'll set per-task

    def run_agent(self, role: str, run_id: int, project_path: str, prompt: str, task_id: int = None) -> Dict[str, Any]:
        """Run agent using Open Interpreter."""
        if not prompt or len(prompt) < 10:
            return {"status": "fail", "summary": f"Invalid prompt for agent: {role}"}
        ...
        try:
            ...
            # Build system message with minimal rules
            hub_url = os.getenv("WORKFLOW_HUB_URL", "http://localhost:8000")
            self.interpreter.system_message = f"""You are a {role.upper()} agent.

WORKSPACE: {project_path}

RULES:
1. Work ONLY within {project_path}
2. Execute code to complete your task
...
```json
{{"status": "pass", "summary": "Brief description", "details": {{}}}}
```

Do NOT continue after outputting the JSON. Your work is complete.
"""

            # Get task context using the abstracted function (works for any provider)
            task_prompt = ""
            if task_id:
                try:
                    from scripts.workflow_tools import format_task_prompt
                    task_prompt = format_task_prompt(task_id, role=role)
                    print(f"  Loaded task context for task {task_id}")
                except Exception as e:
                    print(f"  Could not load task context: {e}")
                    task_prompt = f"Task ID: {task_id}\nRun ID: {run_id}\nWorkspace: {project_path}"

            # Task-specific session persistence
            # Load previous messages for this task if they exist
            session_dir = os.path.join(project_path, ".interpreter", "sessions")
            session_file = os.path.join(session_dir, f"task_{task_id or run_id}.json")

            if os.path.exists(session_file):
                try:
                    with open(session_file, 'r') as f:
                        self.interpreter.messages = json.load(f)
                    print(f"  Loaded session with {len(self.interpreter.messages)} previous messages")
                except Exception as e:
            ...
            max_turns = int(os.getenv("INTERPRETER_MAX_TURNS", "20"))
            self.interpreter.max_turns = max_turns

            # Combine task context with the role-specific prompt
            # Order: task_prompt first (history, tools, criteria, objective), then role prompt
            if task_prompt:
            ...
            # Run the agent
            print(f"  Running Open Interpreter ({role}) with max {max_turns} turns...")

            # Use chat method which returns messages
            response = self.interpreter.chat(full_prompt, display=False)
            ...
            if not report:
                report = {
                    "status": "pass",
                    "summary": f"{role.upper()} agent completed",
                    "details": {}
                }

            report["raw_output"] = output
            report["provider"] = "open-interpreter"

            # Save session for next call on same task (file-based)
            try:
                os.makedirs(session_dir, exist_ok=True)
                with open(session_file, 'w') as f:
                    json.dump(self.interpreter.messages, f)
                print(f"  Saved session with {len(self.interpreter.messages)} messages")
            except Exception as e:
                print(f"  Could not save session: {e}")

            # Also save to database for UI visibility
            try:
                ...
                print(f"  Could not save session to database: {e}")
            ...
            return {
                "status": "fail",
                "summary": f"Error running Open Interpreter: {str(e)}",
                "details": {"error": str(e)}
            }

    def _extract_output(self, response) -> str:
        """Extract text output from interpreter response."""
            ...
            for msg in response:
                if isinstance(msg, dict):
                    if msg.get("type") == "message":
                        ...
                        output_parts.append(f"```{msg.get('format', '')}\n{msg.get('content', '')}\n```")
                    elif msg.get("type") == "console":
                        output_parts.append(f"Output: {msg.get('content', '')}")
    ...
    def _save_session_to_db(self, task_id: int, run_id: int, role: str,
                            messages: list, project_path: str):
        """Save conversation messages to database LLMSession.

        Args:
            task_id: Task ID
            run_id: Run ID
            role: Agent role (pm, dev, qa, security)
            messages: List of message dicts from interpreter
        ...
        try:
            ...
            db = SessionLocal()
            try:
                # Create unique session name for this task/role combination
                session_name = f"task_{task_id}_{role}" if task_id else f"run_{run_id}_{role}"

                # Find or create session
                session = db.query(LLMSession).filter(
                    LLMSession.name == session_name
                ).first()
                ...
                if not session:
                    session = LLMSession(
                        ...
                        model=os.getenv("GOOSE_MODEL", "ai/qwen3-coder"),
                        provider="open-interpreter",
                        created_by=role,
                        messages=[]
                    )
                    db.add(session)

                # Convert interpreter messages to our format
                formatted_messages = []
                for msg in messages:
                    formatted = {
                        ...
                        "ts": msg.get("ts") or msg.get("timestamp") or None
                    }
                    if msg.get("format"):
                        formatted["format"] = msg.get("format")
                ...
                db.commit()
                print(f"  Saved {len(formatted_messages)} messages to database session '{session_name}'")
            ...
            print(f"  Database session save error: {e}")
...
def get_provider() -> AgentProvider:
    ...
    Set AGENT_PROVIDER env var to choose:
    - 'interpreter' or 'open-interpreter': Open Interpreter (recommended - lean, fast)
    - 'goose': Goose CLI (heavy, many extensions)
    ...
    if provider == "mock":
        return MockProvider()
    elif provider in ("interpreter", "open-interpreter", "oi"):
        return OpenInterpreterProvider()
    elif provider == "goose":
        return GooseProvider()
    else:
        # Default to Open Interpreter (simpler, faster)
        print(f"Unknown provider '{AGENT_PROVIDER}', using Open Interpreter")
        return OpenInterpreterProvider()
...
def main():
    ...
    pipeline_parser = subparsers.add_parser("pipeline", help="Run full pipeline for a run (PM → DEV → QA → SEC)")
    ...
    pipeline_parser.add_argument("--max-iterations", type=int, default=10, help="Maximum pipeline iterations")

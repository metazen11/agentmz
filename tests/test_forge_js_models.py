"""Forge tests: create and improve JS files per model."""
...
def test_forge_creates_and_improves_js(model: str):
    if model == "__none__":
    ...
    _log(f"Starting Forge JS create for model={model} file={filename}")

    create_prompt = (
        "You are Forge, a system-agnostic coding agent. "
        f"Create a new file named {filename} in the workspace root. "
        "The file should only contain JavaScript that displays 'hello world' "
        "by setting document.body.textContent and logging to the console. "
        "Use write_file to create the file and do not create any other files."
    )

    env.setdefault("AGENT_CLI_MAX_ITERS", "4")

    create_run = _run_forge(create_prompt, model, workspace, env, timeout=240)
    _log(f"Forge create elapsed={create_run['elapsed']:.2f}s timed_out={create_run['timed_out']}")
    if create_run["timed_out"]:
        _log("Forge timed out while creating JS; checking for partial output.")
    else:
        _log(f"Forge create exit code={create_run['returncode']}")
        if create_run["stdout_lines"]:
            _log(f"Forge create stdout (tail): {create_run['stdout_lines'][-3:]}")
        if create_run["stderr_lines"]:
            _log(f"Forge create stderr (tail): {create_run['stderr_lines'][-3:]}")
        assert create_run["returncode"] == 0, "Forge runner failed on create"
    if not os.path.isfile(target_path):
        ...
        strict_prompt = (
            ...
            + filename +
            '","content":"..."} }. '
            "The content must be JavaScript that sets document.body text to 'hello world' "
            "and logs it. Do not include any prose."
        )
        strict_env = env.copy()
    ...
    assert "document.body" in content.lower()

    _log(f"Starting Forge JS improve for model={model} file={filename}")

    improve_prompt = (
        "You are Forge, a system-agnostic coding agent. "
        f"Improve the existing file {filename} without changing the filename. "
        "Keep the console log and hello world text. "
        "Wrap the logic in DOMContentLoaded and avoid overwriting the entire body. "
        "Create a dedicated element (e.g., div) and append it to the body. "
        "Use read_file first, then apply_patch to update the file."
    )
    ...
    if improve_run["timed_out"]:
        _log("Forge timed out while improving JS; re-checking file.")
    else:
    ...
    with open(target_path, "r", encoding="utf-8", errors="ignore") as handle:
        updated = handle.read().lower()

    assert "hello world" in updated
    assert "console.log" in updated
    assert "domcontentloaded" in updated
    assert "createelement" in updated
    assert "append" in updated
